    ...
    <body>
        ...
        <ul class="nav-links">
            ...
            <div class="main">
                ...
                <div id="main_body">
                    <?php
                    if ($res = $mysqli->query('SELECT * FROM shopping_list INNER JOIN user USING (iduser) INNER JOIN ingredient USING (ingredient_id) INNER JOIN unit USING (unit_id) WHERE iduser=' . $_COOKIE["idUser"])) {
                        if ($res->data_seek(0)) {
                            // loop through the results...
                            echo("<h2><u>Shopping List</u></h2>");
                            while ($row = $res->fetch_assoc()) {
                                // output heading
                                $ingredients = $row['amount'] . " " . $row['unit_name'] . " " . $row['ingredient_name'];
                                $remove = '<a title="Remove From Shopping List"href="shopping-list-delete.php?id=' . $row['shopping_list_id'] . '">&#10060</a>';
                                echo "<h3>" . $remove . $ingredients . "</h3>";
                            }
                        } else {
                            // if there are no results
                            echo "<h1><br>Your Shopping List Is Empty</h1>";
                        }
                    } else {
                ...
                <form action="shopping-list-add.php" method="post" enctype="multipart/form-data">
