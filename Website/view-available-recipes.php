        ...
        <div id="main_body">
            <div id="advert_recipes">
                <div class="recipes">
                    ...
                    if ($res = $mysqli->query('SELECT * from recipe r WHERE NOT EXISTS (SELECT 1 FROM recipe_ingredient rp LEFT JOIN pantry p ON rp.ingredient_id = p.ingredient_id AND p.iduser = ' . $_COOKIE["idUser"] . ' WHERE rp.recipe_id = r.recipe_id AND p.ingredient_id IS NULL)')) {
                        echo "<h1>Available Recipes</h1>\n";
                        if ($res->data_seek(0)) {
                            ...
                            echo "<br>No Available Recipes<br><br>You need to add ingredients to your pantry for recipes to become available";
                        }
                    } else {
